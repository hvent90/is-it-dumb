...
This document outlines the requirements for a web application designed to track and aggregate user-reported issues with Large Language Models (LLMs). The product's vision is to become the definitive source for understanding the real-time, crowd-sourced performance and sentiment of popular LLMs, similar to how "downforeveryoneorjustme.com" serves as a status checker for websites.
